# A Skid Idc With Minor Changes
# removes broke modules and the nsfw features!
import asyncio
import datetime
import functools
import io
...
async def mee6(ctx, param=None):
    ...
    while WoBo.mee6 is True:
        ...
        await WoBo.get_channel(WoBo.mee6_channel).send(random.choice(sentences), delete_after=0.1)
        await asyncio.sleep(60)


@WoBo.command(aliases=['slotsniper', "slotbotsniper"])
async def slotbot(ctx, param=None):
    await ctx.message.delete()
    WoBo.slotbot_sniper = False
    if str(param).lower() == 'true' or str(param).lower() == 'on':
        WoBo.slotbot_sniper = True
    elif str(param).lower() == 'false' or str(param).lower() == 'off':
        WoBo.slotbot_sniper = False
...
@WoBo.command(aliases=['giveawaysniper'])
async def giveaway(ctx, param=None):
    await ctx.message.delete()
    WoBo.giveaway_sniper = False
    if str(param).lower() == 'true' or str(param).lower() == 'on':
        WoBo.giveaway_sniper = True
    elif str(param).lower() == 'false' or str(param).lower() == 'off':
        WoBo.giveaway_sniper = False
